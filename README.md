...
evidenced` date depends on), and a per-scan store of the **validated evidence passages** — the same
text that ships in the report, one vector each. Fetched pages are read in memory and never
persisted, and the namespace is purged before each new scan of that company, so stored evidence
never outlives the latest scan.
...
| [`docs/ai-pre-scan-presentation.pptx`](docs/ai-pre-scan-presentation.pptx) · [`docs/presentation.md`](docs/presentation.md) · [`docs/demo-plan.md`](docs/demo-plan.md) · [`docs/elevator-pitch.md`](docs/elevator-pitch.md) | Editable 10-slide deck, presentation canon, demo running order and spoken pitch |
...
```bash
pip install -e '.[dev]' && pytest    # 97 tests, offline, no keys
